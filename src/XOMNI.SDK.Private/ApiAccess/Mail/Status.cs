...
namespace XOMNI.SDK.Private.ApiAccess.Mail
{
    public class Status : ApiAccessBase
    {
        ...
        {
            return HttpProvider.GetAsync<MailSubscription>(GenerateUrl(string.Format(SingleOperationBaseUrl, emailAddress)), credential);
        }

        public Task PutAsync(string emailAddress, MailSubscriptionStatus subscriptionStatus, ApiBasicCredential credential)
        {
            return HttpProvider.PutAsync(GenerateUrl(string.Format(SingleOperationBaseUrl, emailAddress)), new { StatusId = (int)subscriptionStatus }, credential);
        }

        public XOMNIRequestMessage<MailSubscription> CreateGetRequest(string emailAddress, ApiBasicCredential credential)
        {
            return new XOMNIRequestMessage<MailSubscription>(HttpProvider.CreateGetRequest(GenerateUrl(string.Format(SingleOperationBaseUrl, emailAddress)), credential));
        }
        ...
        {
            return new XOMNIRequestMessage(HttpProvider.CreatePutRequest(GenerateUrl(string.Format(SingleOperationBaseUrl, emailAddress)), credential, new { StatusId = (int)subscriptionStatus }));
        }
    }
}
